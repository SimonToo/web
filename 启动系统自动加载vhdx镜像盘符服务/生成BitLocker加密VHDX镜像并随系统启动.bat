...
for /f "skip=2 tokens=1 delims= " %%a in ('manage-bde -on %DRIVE_LETTER%: -RecoveryPassword ^| findstr /C:"-"') do set key=%%a & set "salt=%random%%random%" & call set "data=%%salt%%#%%key%%" & call echo %%data%%>"%temp%\tmp1.txt" & certutil -encode "%temp%\tmp1.txt" "%temp%\tmp2.txt" >nul & (for /f "skip=1 delims=" %%i in ('type "%temp%\tmp2.txt"') do if not "%%i"=="-----END CERTIFICATE-----" echo %%i>>"%temp%\tmp3.txt") & certutil -f -encode "%temp%\tmp3.txt" "%VHDX_PATH:~0,-5%.key" >nul & del "%temp%\tmp1.txt" "%temp%\tmp2.txt" "%temp%\tmp3.txt" 2>nul & attrib +h %VHDX_PATH:~0,-5%.key

::任务计划添加自启动解BitLocker锁
echo ^<?xml version="1.0" encoding="UTF-16"?^>>"%temp%\%VDService%.xml" & echo ^<Task version="1.2" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task"^>>>"%temp%\%VDService%.xml" & echo   ^<Triggers^>>>"%temp%\%VDService%.xml" & echo     ^<BootTrigger /^>>>"%temp%\%VDService%.xml" & echo   ^</Triggers^>>>"%temp%\%VDService%.xml" & echo   ^<Principals^>>>"%temp%\%VDService%.xml" & echo     ^<Principal id="System"^>>>"%temp%\%VDService%.xml" & echo       ^<UserId^>S-1-5-18^</UserId^>>>"%temp%\%VDService%.xml" & echo       ^<RunLevel^>HighestAvailable^</RunLevel^>>>"%temp%\%VDService%.xml" & echo     ^</Principal^>>>"%temp%\%VDService%.xml" & echo   ^</Principals^>>>"%temp%\%VDService%.xml" & echo   ^<Actions Context="System"^>>>"%temp%\%VDService%.xml" & echo     ^<Exec^>>>"%temp%\%VDService%.xml" & echo       ^<Command^>cmd.exe^</Command^>>>"%temp%\%VDService%.xml" & echo       ^<Arguments^>/c @echo off ^&amp; (echo select vdisk file="%VHDX_PATH%"^&amp;echo attach vdisk^&amp;echo exit)^|diskpart ^&amp; certutil -decode "%VHDX_PATH:~0,-5%.key" "%%temp%%\tmp1.txt" ^&gt;nul ^&amp; certutil -decode "%%temp%%\tmp1.txt" "%%temp%%\tmp2.txt" ^&gt;nul ^&amp; for /f "delims=" %%i in ('type "%%temp%%\tmp2.txt"') do set "data=%%i" ^&amp; call set "key=%%data:*#=%%" ^&amp; del "%%temp%%\tmp1.txt" "%%temp%%\tmp2.txt" 2^&gt;nul ^&amp; for /f "tokens=1,2,3,4,5" %%a in ('^^(echo select vdisk file="%VHDX_PATH%"^^^&amp;echo detail disk^^^&amp;echo exit^^)^^^|diskpart^^^|find "正常"') do call manage-bde -unlock %%c: -rp %%key%% ^&amp; exit^</Arguments^>>>"%temp%\%VDService%.xml" & echo     ^</Exec^>>>"%temp%\%VDService%.xml" & echo   ^</Actions^>>>"%temp%\%VDService%.xml" & echo ^</Task^>>>"%temp%\%VDService%.xml"
